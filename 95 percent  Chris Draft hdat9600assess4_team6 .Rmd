...
1.Getting an overview of dataset
```{r}
...
Getting an overview of dataset, and there are 2061 rows, 120 variables and 11193 missing values at the very beginning



2.Remove all variables that have more than 20% missing value and check the dataset again
```{r}
...
ls(icu_removed_na) 
```
There are 1855 rows and 103 variables left

3. Check the summary of all variables
```{r}
summary(icu_removed_na)
```

4. the SOFA score should be in the range between 0-24, and length of stay shouldn't be negative so delete 
```{r}
...
**P value between all predictor and in_hospital_death**
```{r}
...
Pvalue_each <- tbl_summary(data_frame, by=hospital_death) %>%
          add_p() %>%  
...
We will try fit every significant predictor at the beginning 
```{r}
#general look
...
The significant variables from mod1 will goes to mod2
```{r}
...
The significant variables from mod2 will goes to mod3, and ICU has been removed
```{r}
...
**P value is significant so the model with more variables is better to avoid over fitting, `mod2` also has the lowest AIC(1224.3) which is the best model**

From the `mod2` we know that the risk of patients died in the hosptial has huge connection between SAPS1,Age,AST_max, Bilirubin_min, BUN_min, Cholesterol_max, Cholesterol_min, GCS_max, Platelets_diff, Platelets_max
...
```{r}
#check brier score
predicted <- predict(mod2, type = "response")

BS <- mean(predicted - icu_removed_na$in_hospital_death ^2)
BS


```

**The binnedplot can be used to assess both the overall fit of regression models for binary outcomes and the inclusion of continuous variable and it looks OK. The Brier score of 0 means perfect accuracy, and a Brier score of 1 means perfect inaccuracy. Brier score 1.460868e-15 is infinitely close to 0 which is good**




*Fit to unimputed dataset*
```{r}
...
summary(mod4)
```
mod2:SAPS1 + Age + AST_max + Bilirubin_min + BUN_min + Cholesterol_max + Cholesterol_min + GCS_max + ICUType + pH_diff + Platelets_diff + Platelets_max
mod4:SAPS1 + Age + BUN_min  + GCS_max + ICUType + Platelets_diff + Platelets_max

The most difference is if we decide to remove the variable that more than 20% are missing will lead to more variables are not able to keep in the data set(icu_patients_df0), for example, Bilirubin_min, Cholesterol_max, Cholesterol_min, pH_diff have all gone. Although the rest of variables of mod4 are not exactly the same with the mod2 but they are still significant.(simliar )
...
```{r}
task2 <- coxph(Surv(Days, Status) ~ .-RecordID -in_hospital_death -Length_of_stay -Age, data = icu_removed_na)
summary(task2)
```
