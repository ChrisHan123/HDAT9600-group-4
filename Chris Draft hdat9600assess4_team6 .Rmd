...
```{r}
#remove all variables that have more than 20% missing value(or maybe don't need to)
icu_remove_na <- icu_patients_df1[colSums(is.na(icu_patients_df1))/nrow(icu_patients_df1) < .2]
icu_remove_na <- na.omit(icu_remove_na)
...
summary(icu_remove_na)
```

```{r}
#general look
mod1 <- glm(in_hospital_death ~ SAPS1 + SOFA + Albumin_max + Albumin_min + ALP_max + ALP_min + ALT_max + ALT_min + AST_max + AST_min + Bilirubin_max + Bilirubin_min + BUN_diff + BUN_max + BUN_min + Cholesterol_diff + Creatinine_diff + Creatinine_max + Creatinine_min + FiO2_min + GCS_max + GCS_min + Glucose_diff + Glucose_max + HCO3_diff + HCO3_max + HCO3_min + HCT_max + HR_diff + HR_max + HR_min + ICUType + K_diff + K_min + Lactate_diff + Lactate_max + Lactate_min + Na_diff + PaCO2_diff + PaCO2_max + PaCO2_min + PaO2_diff + PaO2_max + PaO2_min + Platelets_diff + Platelets_max + RespRate_max + RespRate_min + SaO2_diff + Temp_diff + Temp_min + TroponinI_min + Urine_diff + Urine_max + Urine_min + WBC_diff + WBC_min + Weight_max + Weight_min, family = binomial(link = 'logit'), data=icu_remove_na)
...
mod2 <- glm(in_hospital_death ~ SOFA + BUN_min + GCS_max + HR_min + ICUType + Platelets_max, family = binomial(link = 'logit'), data=icu_remove_na)
summary(mod2)
```

```{r}
#sig in Mod2 goes to mod3
mod3 <- glm(in_hospital_death ~ SOFA + BUN_min + GCS_max + Platelets_max, family = binomial(link = 'logit'), data=icu_remove_na)
summary(mod3)
```

```{r}
#Anova test to test which model is better
compared <- anova(mod2, mod3)
compared
```


```{r}
#Checking AIC value,mod2(1335.4) < mod3(1359.8), looks like mod2 is better
plot(mod2)
```
...
```{r}
#check brier score
predicted_pro <- predict(mod2, type = "response")

Brier_score <- mean(predicted_pro - icu_remove_na$in_hospital_death ^2)
Brier_score

icu_remove_na %>%mutate(predprob=predict(mod2, type="response")) -> icu_remove_na


roc(icu_remove_na$in_hospital_death, icu_remove_na$predprob, plot = TRUE, percent=TRUE)
```
...
```{r}
task2 <- coxph(Surv(Days, Status) ~ .-RecordID -in_hospital_death -Length_of_stay -Age, data = icu_remove_na)
summary(task2)
```
