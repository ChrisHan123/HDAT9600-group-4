...
mod2 <- glm(in_hospital_death ~ SAPS1 + Age + AST_max + Bilirubin_min + BUN_min + Cholesterol_max + Cholesterol_min + GCS_max + ICUType + pH_diff + Platelets_diff + Platelets_diff, family = binomial(link = 'logit'), data=icu_removed_na)
summary(mod2)
```

```{r}
#sig in Mod2 goes to mod3, ICU removed
mod3 <- glm(in_hospital_death ~ SAPS1 + Age + AST_max + Bilirubin_min + BUN_min + Cholesterol_max + Cholesterol_min + GCS_max, family = binomial(link = 'logit'), data=icu_removed_na)
summary(mod3)
```
